...
static int jzfb_probe(struct platform_device *pdev)
{
	...
	/*  Must be turned on lcd/cgu_lpc/ahb0 3 clk  */
	jzfb->clk_div = devm_clk_get(jzfb->dev ,"div_lcd");
	if(IS_ERR(jzfb->clk)){
		dev_err(&pdev->dev,"slcd clk get Failed!\n");
		ret = -EINVAL;
		goto err_get_jzfb_clk;
	}
	jzfb->ahb0_gate = devm_clk_get(jzfb->dev, "gate_lcd");
	if (IS_ERR(jzfb->ahb0_gate)) {
		dev_err(&pdev->dev, "jzfb clk get failed!\n");
		ret = -EINVAL;
        goto err_get_ahb_clk;
	}

	ret = clk_set_parent(jzfb->clk_mux, clk_get(NULL, clk_name));
    if (ret){
        printk("clk_set_parent failed!!! parent name = %s\n", clk_name);
    }
    jzfb->clk = devm_clk_get(jzfb->dev, "gate_lcd");
	if (IS_ERR(jzfb->clk)) {
		dev_err(&pdev->dev, "jzfb clk get failed!\n");
		goto err_get_jzfb_clk;
	}
	...
	if ((rc = clk_prepare_enable(jzfb->clk)) < 0) {
		dev_err(&pdev->dev, "Enable jzfb gate clk failed\n");
		goto err_prepare_jzfb_clk;
	}
	if ((rc = clk_prepare_enable(jzfb->ahb0_gate)) < 0) {
...
err_get_jzfb_clk:
	...
	devm_clk_put(&pdev->dev, jzfb->clk_div);
	devm_clk_put(&pdev->dev, jzfb->ahb0_gate);
err_get_ahb_clk:
	devm_clk_put(&pdev->dev, jzfb->ahb0_gate);
err_prepare_jzfb_clk:
	clk_disable_unprepare(jzfb->clk);
err_prepare_ahb_clk:
	clk_disable_unprepare(jzfb->ahb0_gate);
err_free_file:
