...
#ifdef CONFIG_OF
static const struct of_device_id bcmsdh_sdio_of_match_table[] = {
	{ .compatible = "android,bcmdhd_wlan" },
	{ }
};

static int bcmdhd_probe_of(struct device *dev, wifi_adapter_info_t *adapter)
{
	struct device_node *np = dev->of_node;
	unsigned int flags, gpio;

	if (!np || !of_match_node(bcmsdh_sdio_of_match_table, np)) {
		return -ENODATA;
	}
	printk("%s---------------adapter->bcm_data.wifi_reset %d\n", __func__, __LINE__);
	gpio = of_get_named_gpio_flags(np, "bcmdhd,irq", 0, &flags);
	if(gpio_is_valid(gpio)) {
		adapter->irq_num = gpio;
		adapter->intr_flags = flags & OF_GPIO_ACTIVE_LOW;
	}
...
wifi_adapter_info_t* dhd_wifi_platform_get_adapter(struct device *dev, uint32 bus_type, uint32 bus_num, uint32 slot_num)
{
	int i;
