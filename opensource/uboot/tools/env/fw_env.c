...
int fw_env_open(void)
{
	...
	if (!HaveRedundEnv) {
		if (*environment.crc == 0xffffffff)
		{
			fprintf(stderr,
					"Warning: The environment is empty");
			goto load_one_default;
		}

		if (!crc0_ok) {
			fprintf (stderr,
				"Warning: Bad CRC0, using default environment");
load_one_default:
			printf(" ... loading default environment ... ");
			dev_current = 0;
		...
		if (redundant->crc == 0xffffffff && *environment.crc == 0xffffffff)
		{
			fprintf(stderr,
					"Warning: The environment is empty");
load_double_default:
			printf(" ... loading default environment ");
			dev_current = 0;
			memcpy(redundant->data, default_environment, sizeof default_environment);
			redundant->flags = 0;
			redundant->crc = crc32(0, (uint8_t *)redundant->data, ENV_SIZE);
			if (flash_io(O_RDWR))
			{
				printf("ERROR !!!\n");
				return -1;
		...
		if (crc0_ok && !crc1_ok) {
			...
			fprintf(stderr,
					"Warning: Bad CRC0&1, using default environment");
			goto load_double_default;
		} else {
			switch (environment.flag_scheme) {
			case FLAG_BOOLEAN:
