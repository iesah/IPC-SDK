...
struct spi_flash {
	...
	int		(*read)(unsigned char sfc_index,struct spi_flash *flash, u32 offset,
				size_t len, void *buf);
	int		(*write)(unsigned char sfc_index,struct spi_flash *flash, u32 offset,
				size_t len, const void *buf);
	int		(*erase)(unsigned char sfc_index,struct spi_flash *flash, u32 offset,
				size_t len);
};
...
static inline int spi_flash_read(unsigned char sfc_index,struct spi_flash *flash, u32 offset,
		size_t len, void *buf)
{
	return flash->read(sfc_index,flash, offset, len, buf);
}

static inline int spi_flash_write(unsigned char sfc_index,struct spi_flash *flash, u32 offset,
		size_t len, const void *buf)
{
	return flash->write(sfc_index,flash, offset, len, buf);
}

static inline int spi_flash_erase(unsigned char sfc_index,struct spi_flash *flash, u32 offset,
		size_t len)
{
	return flash->erase(sfc_index,flash, offset, len);
}
