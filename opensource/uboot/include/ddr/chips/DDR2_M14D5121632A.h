...
#define DDR_tWTR	(DDR_WL + DDR_tCCD + DDR__ns(8))  /* WRITE to READ command delay. */
/*
 * DDR2 controller timing2 register
 */
#define DDR_tRFC    DDR__ns(130) /* ns,  AUTO-REFRESH command period. */
#define DDR_tMINSR	DDR__ns(0) /* Minimum Self-Refresh / Deep-Power-Down */
#define DDR_tXP		DDR__tck(5)    /* tCK EXIT-POWER-DOWN to next valid command period: 1 to 8 tCK. */
#define DDR_tMRD	DDR__tck(5)   /* unit: tCK. Load-Mode-Register to next valid command period: 1 to 4 tCK */
...
#define DDR_tRTW	(DDR_RL - DDR_WL + DDR__tck(5)) /* 4 in case of BL=4, 6 in case of BL=8 */
#define DDR_tFAW	DDR__ns(60)     /* Four bank activate period, ns */
#define DDR_tCKE	DDR__tck(5)      /* CKE minimum pulse width, tCK */
#define DDR_tCKESR	DDR__tck(0)      /* CKE minimum pulse width, tCK */
#define DDR_tRDLAT  (DDR_RL - DDR__tck(3))
#define DDR_tWDLAT  (DDR_WL - DDR__tck(1))	/*(DDR_tWL - 1)*/
#define DDR_tXARD	DDR__tck(5)     /* DDR2 only: Exit active power down to read command , tCK*/
...
#define DDR_tCKSRE	DDR__tck(0)   /* DDR2 no:Valid Clock Requirement after Self Refresh Entry or Power-Down Entry */
/*
