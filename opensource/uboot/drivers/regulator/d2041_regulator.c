...
static int d2041_set_voltage(struct regulator* regulator, int min_uV, int max_uV);
...
int d2041_i2c_read( unsigned int start_reg, unsigned char *dest)
{
	int ret = -1;
...
static int d2041_set_voltage(struct regulator* regulator, int min_uV, int max_uV)
{
	...
	ret = d2041_i2c_read(D2041_SUPPLY_REG, &val);
	...
	ret = d2041_i2c_read(reg_num, &val);
...
int d2041_shutdown(void)
{
  ...
  d2041_i2c_write(D2041_POWERCONT_REG, 1, &dst);
    ...
    dst = 0;
    d2041_i2c_read(D2041_CONTROLB_REG, &dst);
