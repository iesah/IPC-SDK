		...
		int boot_progress)
{
	...
	//load_buf = malloc(load, image_len);
	image_buf = map_sysmem(image_start, image_len);
	//printf("comp %d load 0x%x start=0x%x end=0x%x image_start=0x%x image_len=%d\n", comp, load, blob_start, blob_end, image_start, image_len);
	switch (comp) {
	case IH_COMP_NONE:
...
#if 0
		printf("lzma hardware decompress [outlen = %d] \n", ret);
		int i = 0;
		int *pcLoad = (int *)load_buf;
		printf("start 64 ints: ");
		for(i = 0; i < 64; i++){
			printf("0x%08X ", *(pcLoad+i));
		}
		printf("\nlast 64 bytes: ");
		for(i = ret-64; i < ret; i++){
			printf("0x%08X ", pcLoad[i]);
		}
		printf("\n");
#endif
	...
	case IMAGE_FORMAT_LEGACY:
		...
		case IH_TYPE_KERNEL_NOLOAD:
			...
			*os_len = image_get_data_size(hdr);
			//printf("boot_get_kernel os_data:0x%x, os_len=%d\n", *os_data, *os_len);
			break;
		case IH_TYPE_MULTI:
