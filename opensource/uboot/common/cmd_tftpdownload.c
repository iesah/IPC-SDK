...
static int update_to_flash(unsigned int size)
{
	...
	printf("flash erase...\n");
	rc = flash->erase(index,flash, start, size);
	if (rc) {
	...
	printf("flash write...\n");
	rc = flash->write(index,flash, start, size, buf);
	if (rc) {
...
int do_tftp_download (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	flash = spi_flash_probe(index,0, 0, CONFIG_SF_DEFAULT_SPEED, 0x3);
	if (!flash) {
