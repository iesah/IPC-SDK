...
static int vo_probe(struct platform_device *pdev)
{
	...
	vo->clk_div = clk_get(vo->dev,"cgu_bt0");
	if(IS_ERR(vo->clk_div)){
		dev_err(vo->dev,"clk_gate get failed\n");
		ret = PTR_ERR(vo->clk_div);
		goto err_get_vo_clk;
	}
	vo->clk = clk_get(vo->dev, "vo");
    if (IS_ERR(vo->clk)) {
        dev_err(vo->dev, "clk get failed\n");
        ret = PTR_ERR(vo->clk);
        goto err_get_vo_clk;
    }
    ...
    printk("----reg:0x10000020 value=0x%08x  (26bit) Clock Gate Register0\n",
           *(volatile unsigned int *)0xb0000020);
    printk("----reg:0x100000B8 value=0x%08x  BT0 pixclock \n",
...
static int __init vodev_init(void)
{
    int ret = 0;  
    ret = platform_device_register(&jz_vo_device);
	if(ret){
		printk("%s platform_device_register failed:ret=0x%x\n",__func__,ret);
		return -1;
	}
	ret = platform_driver_register(&jz_vo_driver);
    if(ret){
...
static void __exit vodev_exit(void)
{
       VO_DEBUG("%s\n", __func__);
       platform_driver_unregister(&jz_vo_driver);
}

module_init(vodev_init);
module_exit(vodev_exit);

MODULE_DESCRIPTION("JZ VO driver");
MODULE_AUTHOR("Ingenic <aa@ingenic.cn>");
MODULE_LICENSE("GPL");
