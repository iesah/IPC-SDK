...
static int cpu_ccu_test_func(void * param)
{
	...
	cscr = get_ccu_cscr(); //core sleep control register

	cssr = get_ccu_cssr(); //core sleep status register

	csrr = get_ccu_csrr(); //core software reset register

	rer = get_ccu_rer();   //reset entry register
