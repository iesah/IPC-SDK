...
#define DEF_CLK(N,FLAG)						\
	...
	DEF_CLK(AIC,  		GATE(11) | PARENT(CGU_I2S_MIC)),
	...
	DEF_CLK(CGU_BSCALER,	CGU(CGU_BSCALER) | PARENT(H0CLK)),
...
struct clk *clk_get(struct device *dev, const char *id)
{
	...
	for(i = 1; i < ARRAY_SIZE(clk_srcs); i++) {
		...
		if(clk_srcs[i].name == NULL) continue;
