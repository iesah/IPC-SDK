...
static int truly_slcd240240_power_initialize(struct lcdc_gpio_struct *gpio)
{
	...
	ret = gpio_request(GPIO_PC(18), "lcd_bl");
	if (ret) {
		printk(KERN_ERR "can's request lcd rst\n");
		return ret;
	}
	ret = gpio_request(GPIO_PD(30), "lcd_pwm");
	if (ret) {
		printk(KERN_ERR "can's request lcd rst\n");
		return ret;
	}
	gpio_direction_output(GPIO_PC(18), 1);
	gpio_direction_output(GPIO_PD(30), 1);
#endif
...
struct lcd_callback_ops truly_slcd240240_ops = {
...
struct fb_videomode jzfb_truly_240_240_videomode = {
	.name = "240x240",
	.refresh = 60,
	.xres = 240,
	.yres = 240,
...
struct jzfb_smart_config truly_240_240_cfg = {
	.frm_md = 0,
	.rdy_anti_jit = 0,
	.te_anti_jit = 1,
	.te_md = 0,
