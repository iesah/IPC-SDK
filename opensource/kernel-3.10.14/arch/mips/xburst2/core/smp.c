...
void percpu_mailbox_init(int cpu)
{
	...
	*__this_cpu_ptr(smp_core.percpu_mailbox) = &smp_core.mailbox[cpu];
...
//	int ret;
	...
	pr_debug("#### now starting init for cpu : %d\n", cpu);
	...
	percpu_timerevent_init(cpu_num);
 ...
 * loop
 */
static void __cpuinit xburst2_smp_finish(void)
{
	int cpu = smp_processor_id();
	migrate_irqs(cpu);
	xburst2_ccu_showregs();
...
static void __init xburst2_smp_setup(void)
{
	...
	pr_debug("[SMP] Slave CPU(s) %i available.\n", num);
}
...
{
	int ret;
	pr_debug("[SMP] Prepare %d cores., cpu: %d\n", max_cpus, smp_processor_id());//qiao
...
retry:
	...
	if(timeout == 0){
		pr_err("SMP[%d] action:%d will reenter, mailbox:%x \n",cpu,action, readl(mailbox->iobase));
		dump_stack();
	}
	if(irqs_disabled()){
		dump_stack();
