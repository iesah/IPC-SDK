			  ...
			  struct pt_watch_regs __user *addr)
{
	...
	if (!cpu_has_watch || raw_current_cpu_data.watch_reg_use_cnt == 0)
		return -EIO;
...
#define WATCH_STYLE mips64
	...
	__put_user(raw_current_cpu_data.watch_reg_use_cnt,
		   &addr->WATCH_STYLE.num_valid);
	for (i = 0; i < raw_current_cpu_data.watch_reg_use_cnt; i++) {
		__put_user(child->thread.watch.mips3264.watchlo[i],
			   &addr->WATCH_STYLE.watchlo[i]);
		__put_user(child->thread.watch.mips3264.watchhi[i] & 0xfff,
			   &addr->WATCH_STYLE.watchhi[i]);
		__put_user(raw_current_cpu_data.watch_reg_masks[i],
