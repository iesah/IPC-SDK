...
static int __init ieee754_setup(char *s)
{
	...
	if (!(current_cpu_data.options & MIPS_CPU_FPU))
		cpu_set_nofpu_2008(&current_cpu_data);
	cpu_set_nan_2008(&current_cpu_data);
...
static inline void cpu_probe_ingenic(struct cpuinfo_mips *c, unsigned int cpu)
{
	...
	if((c->processor_id & PRID_INGENIC_CORE_MASK) == PRID_INGENIC_CORE_XBURST) {
	...
	} else if((c->processor_id & PRID_INGENIC_CORE_MASK) == PRID_INGENIC_CORE_XBURST_II) {
		__cpu_name[cpu] = "Ingenic Xburst@II";
	} else {
		pr_err("Unsupported Processor Id,: %x\n", c->processor_id);
	}
...
__cpuinit void cpu_report(void)
{
	...
	if (cpu_has_msa)
		printk(KERN_INFO "MSA revision is: %08x\n", c->msa_id);
}
