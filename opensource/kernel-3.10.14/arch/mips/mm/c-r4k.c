...
#if !defined(CONFIG_MIPS_MT_SMP) && !defined(CONFIG_MIPS_MT_SMTC) && !defined(CONFIG_MACH_XBURST2)
...
static void r4k_dma_cache_wback_inv(unsigned long addr, unsigned long size)
{
	...
	__sync();	//prevent cpu core migeration.
	preempt_enable();
...
static char *way_string[] __cpuinitdata = { NULL, "direct mapped", "2-way",
	"3-way", "4-way", "5-way", "6-way", "7-way", "8-way", "9-way", "10-way",
	"11-way", "12-way", "13-way", "14-way", "15-way", "16-way"
};
...
static void __cpuinit probe_pcache(void)
{
	...
	default:
		...
		c->options |= MIPS_CPU_PREFETCH;
		printk("%s(%d):config=%x, config1=%lx, prid=%x, c->dcache(linesz=%d,sets=%d,ways=%d,waybit=%d), config11_6=%x\n",
				__func__, __LINE__, config, config1, prid,
				c->dcache.linesz, c->dcache.sets, c->dcache.ways, c->dcache.waybit,
				__read_32bit_c0_register($11, 6));
		break;
	}
